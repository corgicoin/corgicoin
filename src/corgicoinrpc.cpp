...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "CorgiCoin server has now stopped running!";
}
...
CBitcoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    if (account.vchPubKey.IsValid())
    {
        ...
        for (const auto& walletItem : pwalletMain->mapWallet)
        {
            if (!account.vchPubKey.IsValid())
                break;
            const CWalletTx& wtx = walletItem.second;
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current CorgiCoin address for receiving payments to this account.");
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress <corgicoinaddress> <amount> [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <corgicoinaddress> [minconf=1]\n"
            "Returns the total amount received by <corgicoinaddress> in transactions with at least [minconf] confirmations.");
...
Value getbalance(const Array& params, bool fHelp)
{
    ...
    if (params[0].get_str() == "*") {
        ...
        for (const auto& item : pwalletMain->mapWallet)
        {
            ...
            for (const std::pair<CTxDestination,int64>& r : listSent)
                nBalance -= r.second;
            nBalance -= allFee;
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <tocorgicoinaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    for (const Pair& s : sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid CorgiCoin address:")+s.name_);
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Return information about <corgicoinaddress>.");
...
//
// JSON-RPC protocol.  CorgiCoin speaks version 1.0 for maximum compatibility,
...
public:
    ...
    bool connect(const std::string& server, const std::string& port)
    {
        ...
        auto endpoint_iterator = resolver.resolve(server.c_str(), port.c_str());
        decltype(endpoint_iterator) end;
        boost::system::error_code error = asio::error::host_not_found;
        for (const auto& ep : endpoint_iterator)
        {
...
void ThreadRPCServer2(void* parg)
{
    ...
    if (mapArgs["-rpcpassword"] == "")
    {
        ...
        string strWhatAmI = "To use corgicoind";
    ...
    ip::tcp::endpoint endpoint(bindAddress, GetArg("-rpcport", 62555));
    ...
    try
    {
        ...
        std::weak_ptr<ip::tcp::acceptor> weak_acceptor(acceptor);
        ...
        if (loopback || v6_only_error)
        {
            ...
            RPCListen(acceptor, context, fUseSSL);
            // Cancel outstanding listen-requests for this acceptor when shutting down
            std::weak_ptr<ip::tcp::acceptor> weak_acceptor2(acceptor);
            StopRequests.connect([weak_acceptor2]() {
                if (auto a = weak_acceptor2.lock()) a->close();
            });
        }
    }
...
Object CallRPC(const string& strMethod, const Array& params)
{
    ...
    if (!d.connect(GetArg("-rpcconnect", "127.0.0.1"), GetArg("-rpcport", "62555")))
