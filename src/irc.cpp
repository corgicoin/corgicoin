...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #corgicoinTEST9\r");
            Send(hSocket, "WHO #corgicoinTEST9\r");
        } else {
            // randomly join #corgicoin00-#corgicoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = GetRandInt(50);

            Send(hSocket, strprintf("JOIN #corgicoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #corgicoin%02d\r", channel_number).c_str());
        }
