...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        {
            // First part of help message is specific to corgicoind / RPC client
            std::string strUsage = _("CorgiCoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  corgicoind [options]                     " + "\n" +
                  "  corgicoind [options] <command> [params]  " + _("Send command to -server or corgicoind") + "\n" +
                  "  corgicoind [options] help                " + _("List commands") + "\n" +
                  "  corgicoind [options] help <command>      " + _("Get help for a command") + "\n";
...
int main(int argc, char* argv[])
{
    ...
    // Connect corgicoind signal handlers
    noui_connect();
...
{
    uiInterface.ThreadSafeMessageBox(str, _("CorgiCoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("CorgiCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: corgicoin.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: corgicoind.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 62556 or testnet: 64556)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 62555)") + "\n" +
...
bool AppInit2()
{
    ...
    InitLogging(GetArg("-debug", ""), GetBoolArg("-printtoconsole"), GetBoolArg("-logtimestamps"));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  CorgiCoin is probably already running."), GetDataDir().string().c_str()));
    ...
    LogPrintf("\n");
    LogPrintf("*********************************************************************\n");
    LogPrintf("*** CRITICAL SECURITY WARNING: OpenSSL 1.0.x DETECTED            ***\n");
    LogPrintf("*** This version is INSECURE and should NOT be used in production ***\n");
    LogPrintf("*** Please upgrade to OpenSSL 3.0+ immediately                    ***\n");
    LogPrintf("*********************************************************************\n");
    LogPrintf("\n");
#endif
    ...
    // requested to kill corgicoin-qt during the last operation. If so, exit.
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        if (nLoadWalletRet == DB_CORRUPT)
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of CorgiCoin") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart CorgiCoin to complete") << "\n";
    ...
    //// debug print
    LogPrintf("mapBlockIndex.size() = %d\n",   mapBlockIndex.size());
    LogPrintf("nBestHeight = %d\n",            nBestHeight);
    LogPrintf("setKeyPool.size() = %d\n",      pwalletMain->setKeyPool.size());
    LogPrintf("mapWallet.size() = %d\n",       pwalletMain->mapWallet.size());
    LogPrintf("mapAddressBook.size() = %d\n",  pwalletMain->mapAddressBook.size());
