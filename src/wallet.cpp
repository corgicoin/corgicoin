...
bool CWallet::AddToWallet(const CWalletTx& wtxIn)
{
    uint256 hash = wtxIn.GetHash();
    {
        ...
        pair<map<uint256, CWalletTx>::iterator, bool> ret = mapWallet.insert({hash, wtxIn});
                           ...
                           list<pair<CTxDestination, int64> >& listSent, int64& nFee, string& strSentAccount) const
{
    ...
    for (const CTxOut& txout : vout)
    {
        ...
        if (nDebit > 0)
            listSent.emplace_back(address, txout.nValue);

        if (pwallet->IsMine(txout))
            listReceived.emplace_back(address, txout.nValue);
    }
    ...
    while (fRepeat)
    {
        ...
        for (std::pair<const uint256, CWalletTx>& item : mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                {
                    LogPrintf("ERROR: ReacceptWalletTransactions() : txindex.vSpent.size() %d != wtx.vout.size() %d\n", txindex.vSpent.size(), wtx.vout.size());
                    continue;
                }
        ...
        for (const auto& item : mapWallet)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
                if (!(pcoin->IsSpent(i)) && IsMine(pcoin->vout[i]) && pcoin->vout[i].nValue >= nMinimumInputValue)
                    vCoins.emplace_back(pcoin, i, pcoin->GetDepthInMainChain());
        }
    }
}
                                 ...
                                 set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    for (COutput output : vCoins)
    {
        ...
        pair<int64,pair<const CWalletTx*,unsigned int> > coin = {n, {pcoin, i}};
...
bool CWallet::CreateTransaction(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet)
{
    ...
    vecSend.emplace_back(scriptPubKey, nValue);
...
{
    {
        ...
        int64 nKeys = max(GetArg("-keypool", 100), static_cast<int64>(0));
...
{
    {
        ...
        while (setKeyPool.size() < (nTargetSize + 1))
        {
            ...
            LogPrintf("keypool added key %" PRI64d ", size=%d\n", nEnd, setKeyPool.size());
        }
    }
    return true;
}
